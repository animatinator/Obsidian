...
class ParticleEditor(wx.Frame):
    def __init__(self, parent, numframes, keyframes, drawtype, imagepath = None):
        ...
        # The particle display  -------------------------  BOLLOCKED UP CODE BEGINS HERE  -  also see line in OnTimelineChange()
        self.display = pygamedisplay.ObsidianMainDisplay(self.panel, -1)#pygamedisplay.ObsidianParticlePreviewDisplay(self.panel, -1, numframes, keyframes, drawtype)
        # Added shizz begins here
        effect = PyIgnition.ParticleEffect(display = None, pos = (0, 0))
        effect.LoadFromFile("Water.ppe")
        effect.CreatePointGravity(0.4, 0.0, (200, 200))
        effect.sources[0].CreateKeyframe(0, pos = (150, 400))
        effect.sources[0].CreateKeyframe(100, pos = (200, 100))
        effect.sources[0].selected = True
        effect.sources[0].CreateKeyframe(0, initdirectionrandrange = 1.0)
        effect.sources[0].CreateKeyframe(100, initdirectionrandrange = 2.5)
        effect.sources[0].CreateKeyframe(100, initdirection = 2.1)
        for source in effect.sources:
            self.display.AddSource(source)
        for gravity in effect.gravities:
            self.display.AddGravity(gravity)
        for obstacle in effect.obstacles:
            self.display.AddObstacle(obstacle)
        #  -------------------------  BOLLOCKED UP CODE ENDS HERE
    ...
    def OnTimelineChange(self, event):
        self.curframe = self.timeline.curframe
        self.display.curframe = self.curframe ####################################################################
        self.UpdateCurframeValues()
