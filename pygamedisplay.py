...
GRAVITY_NUMSPOKES = 16
GRAVITY_DRAWSIZE = 100
GRAVITY_CENTRE_COLOUR = SOURCE_CENTRE_COLOUR
GRAVITY_LINE_COLOUR = SOURCE_DIRECTIONBOUNDS_COLOUR
...
class ObsidianMainDisplay(PyIgnitionDisplay):
    ...
    def DrawPointGravity(self, gravity, display):
        # Draw lines pointing out in eight directions
        direction = 0.0
        increment = math.pi / (float(GRAVITY_NUMSPOKES) / 2.0)
        radius = float(GRAVITY_DRAWSIZE) / 2.0
        
        while direction < (math.pi * 2.0):
            linex = radius * math.sin(direction)
            liney = radius * math.cos(direction)
            pygame.draw.aaline(display, GRAVITY_LINE_COLOUR, gravity.pos, (gravity.pos[0] + linex, gravity.pos[1] + liney))
            
            direction += increment
        
        # Draw the position of the gravity as a point
        pygame.draw.circle(display, GRAVITY_CENTRE_COLOUR, gravity.pos, 5)
